...
#define SQL_CODE_ITEMS "SELECT `ItemId`, `ItemCount` FROM `world_coded_items` WHERE `CodeId` = {} AND (`ItemClassMask` & {} OR `ItemClassMask` = 0) AND (`ItemRaceMask` & {} OR `ItemRaceMask` = 0)"
...
class Mod_SpecialCode_WorldScript : public WorldScript
{
    ...
    {
        SCEnable = sConfigMgr->GetOption<bool>("SpecialCode.Enable", false);
    }
