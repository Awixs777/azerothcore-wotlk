...
struct boss_magtheridon : public BossAI
{
    ...
    void JustEngagedWith(Unit* who) override
    {
        ...
        scheduler.Schedule(20s, GROUP_EARLY_RELEASE_CHECK, [this](TaskContext /*context*/)
        {
            Talk(SAY_EMOTE_NEARLY);
        }).Schedule(40s, GROUP_EARLY_RELEASE_CHECK, [this](TaskContext /*context*/)
        {
            Talk(SAY_EMOTE_FREE);
            Talk(SAY_FREE);
            _magReleased = true;
        }).Schedule(43s, GROUP_EARLY_RELEASE_CHECK, [this](TaskContext /*context*/)
        {
            ScheduleCombatEvents();
        });
    }
